...
class User(db.Model):
    ...
    topic_count = db.Column(db.Integer, default=0, index=True)
    status_update_created = db.Column(db.Integer, default=0, index=True)
    status_update_comments_created = db.Column(db.Integer, default=0, index=True)
    boops_given = db.Column(db.Integer, default=0, index=True)
    boops_received = db.Column(db.Integer, default=0, index=True)
    profile_visits = db.Column(db.Integer, default=0, index=True)
    ...
    def get_themes(self):
        return SiteTheme.query.order_by("weight").all()
    ...
    def set_password(self, password, rounds=12):
        ...
        self.password_hash = bcrypt.generate_password_hash(password.strip(),rounds).decode('utf-8')
    ...
    def get_recent_notifications(self, count=15):
        _notifications = Notification.query.filter_by(acknowledged=False, user=self) \
            .options(joinedload(Notification.author))\
            .order_by(db.desc(Notification.created)).limit(count)
